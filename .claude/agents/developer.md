...
- **Never run a git command that mutates the working tree, index, or history** — no `git stash` (including `pop`/`drop`/`apply`), `git checkout -- <path>`, `git reset`, `git commit`, `git add`, `git clean`, `git rebase`, or `git merge`. Writing/editing application and test files via `Write`/`Edit` is your job and expected; using git itself to stage, commit, or discard state is not — that's the orchestrator's call, made with the human in the loop. If a task seems to require it (e.g. "clean up your working tree" or "revert that file"), stop and say so in your final report instead of running the command. (This constraint exists because a reviewer agent — a different, read-only role — once ran `git stash`, dropped it before confirming `pop` succeeded, and reverted 9 tracked files; the same risk applies to any Bash-capable role, so it's enforced here too even though your role's blast radius is different.)
