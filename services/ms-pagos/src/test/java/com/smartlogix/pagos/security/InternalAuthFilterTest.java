...
class InternalAuthFilterTest {
    ...
    @Test
    void doFilter_webhookFlow_seExcluyeDelChequeoYContinuaLaCadenaSinCabeceras() throws Exception {

        // ARRANGE — /pagos/webhook/flow no requiere autenticación interna
        // (Flow es un caller externo, sigue con su propia firma HMAC)
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/pagos/webhook/flow");
    ...
    void doFilter_sinCabecerasInternas_retorna401YNoInvocaLaCadena() throws Exception {
        ...
        // ARRANGE
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/pagos/crear");
        MockHttpServletResponse response = new MockHttpServletResponse();
        MockFilterChain chain = new MockFilterChain();
    ...
    void doFilter_firmaForjada_retorna401FirmaInvalida() throws Exception {
        ...
        request.addHeader("X-Internal-Signature", "1".repeat(64));
        MockHttpServletResponse response = new MockHttpServletResponse();
        MockFilterChain chain = new MockFilterChain();
    ...
    @Test
    void doFilter_msPedidosLlamandoCrearPago_retorna403() throws Exception {

        // ARRANGE — /pagos/crear solo acepta api-gateway
        long timestamp = System.currentTimeMillis();
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/pagos/crear");
    ...
    @Test
    void doFilter_apiGatewayLlamandoCrearPago_continuaLaCadena() throws Exception {
