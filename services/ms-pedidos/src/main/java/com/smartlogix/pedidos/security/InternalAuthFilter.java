...
/**
 * Verifica la autenticación interna servicio-a-servicio en /pedidos/** y
 * /sagas/**, y aplica el allowlist de emisor por endpoint (§5.2 del diseño):
 * confirmar-pago/pago-fallido solo aceptan ms-pagos; todo lo demás solo
...
public class InternalAuthFilter implements Filter {
    ...
    private static final Pattern CONFIRMAR_PAGO = Pattern.compile("^/pedidos/[^/]+/confirmar-pago$");
    private static final Pattern PAGO_FALLIDO = Pattern.compile("^/pedidos/[^/]+/pago-fallido$");
    ...
    private boolean isAllowed(HttpServletRequest request, String service) {
        ...
        if ("POST".equals(method) && (CONFIRMAR_PAGO.matcher(uri).matches()
                || PAGO_FALLIDO.matcher(uri).matches())) {
            return "ms-pagos".equals(service);
        }
