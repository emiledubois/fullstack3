...
public class CreateShipmentStep implements SagaStep {
    ...
    @Override
    public String getName() { return "CREAR_ENVIO"; }

    @Override
    public void execute(SagaContext ctx) throws SagaStepException {
        try {
            String tipo    = ctx.getRequest().getTipoPedido() != null
                           && ctx.getRequest().getTipoPedido().equals("INTERNACIONAL")
                             ? "EXPRESS" : "TERRESTRE";
            String destino = ctx.getRequest().getDestino() != null
                           ? ctx.getRequest().getDestino() : "Por definir";
    ...
    public void compensate(SagaContext ctx) {
        if (ctx.getEnvioId() != null) {
            log.warn("[Saga {}] COMPENSACIÓN: Cancelando envío id={}", ctx.getSagaId(), ctx.getEnvioId());
            try {
