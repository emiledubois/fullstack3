...
public class CreateOrderStep implements SagaStep {
    ...
    @Override
    public String getName() { return "CREAR_PEDIDO"; }

    @Override
    public void execute(SagaContext ctx) throws SagaStepException {
        try {
            Pedido pedido = pedidoFactory.crearPedido(ctx.getRequest());
            // Estado CONFIRMADO (en lugar de PENDIENTE) porque el stock ya está reservado
            pedido.setStatus("CONFIRMADO");
            Pedido saved = orderRepository.save(pedido);
            ctx.setPedidoId(saved.getId());
            log.info("[Saga {}] Pedido creado: id={}, status=CONFIRMADO", ctx.getSagaId(), saved.getId());
        } catch (Exception e) {
    ...
    public void compensate(SagaContext ctx) {
        if (ctx.getPedidoId() != null) {
            log.warn("[Saga {}] COMPENSACIÓN: Cancelando pedido id={}", ctx.getSagaId(), ctx.getPedidoId());
            try {
                orderRepository.findById(ctx.getPedidoId()).ifPresent(p -> {
                    p.setStatus("CANCELADO");
                    p.setObservaciones("Cancelado por Saga " + ctx.getSagaId());
