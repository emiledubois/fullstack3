...
/**
 * Verifica la autenticación interna servicio-a-servicio en /inventario/**.
 * Allowlist por endpoint (§5.2 del diseño): reservar/liberar/stock solo
...
public class InternalAuthFilter implements Filter {
    ...
    private static final Pattern RESERVAR = Pattern.compile("^/inventario/[^/]+/reservar$");
    private static final Pattern LIBERAR = Pattern.compile("^/inventario/[^/]+/liberar$");
    private static final Pattern STOCK = Pattern.compile("^/inventario/[^/]+/stock$");
    ...
    private boolean isAllowed(HttpServletRequest request, String service) {
        ...
        boolean esRutaDePedidos =
            ("POST".equals(method) && (RESERVAR.matcher(uri).matches() || LIBERAR.matcher(uri).matches()))
            || ("GET".equals(method) && STOCK.matcher(uri).matches());
        if (esRutaDePedidos) {
